 ...
 */
class Maincategories extends BE_Controller {
	...
	function __construct() {
		...
		parent::__construct( MODULE_CONTROL, 'MAINCATEGORIES' );
	...
	function index() {
		...
		// get rows count
		$this->data['rows_count'] = $this->Maincategory->count_all_by( $conds );

		// get categories
		$this->data['categories'] = $this->Maincategory->get_all_by( $conds , $this->pag['per_page'], $this->uri->segment( 4 ) );
		// load index logic
		parent::index();
	}
	...
	function edit( $id ) {
		...
		// load user
		$this->data['category'] = $this->Maincategory->get_one( $id );
	...
	function save( $id = false ) {
		...
		// prepare cat name
		if ( $this->has_data( 'main_cat_name' )) {
			$data['main_cat_name'] = $this->get_data( 'main_cat_name' );
		}
		...
		// save category
		if ( ! $this->Maincategory->save( $data, $id )) {
	...
	function delete( $category_id ) {
		...
		if ( !$this->ps_delete->delete_maincategory( $category_id )) {
	...
	function is_valid_input( $id = 0 ) {
		...
		$this->form_validation->set_rules( 'main_cat_name', get_msg( 'cat_name' ), $rule);
	...
	{		
		 ...
		 $conds['main_cat_name'] = $name;


		 	if( $cat_id != "") {
		 		// echo "bbbb";die;
				if ( strtolower( $this->Maincategory->get_one( $id )->cat_name ) == strtolower( $name )) {
				// if the name is existing name for that user id,
					return true;
				} 
			} else {
				// echo "aaaa";die;
				if ( $this->Maincategory->exists( ($conds ))) {
	 ...
	 */
	function ajx_exists( $main_cat_id = false )
	{
		// get category name

		$name = $_REQUEST['main_cat_name'];

		if ( $this->is_valid_name( $name, $main_cat_id )) {
	...
	function ajx_publish( $category_id = 0 )
	{
		...
		// save data
		if ( $this->Maincategory->save( $category_data, $category_id )) {
			echo 'true';
		} else {
			echo 'false';
		}
