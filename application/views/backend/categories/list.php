	...
	<?php if ( !empty( $categories ) && count( $categories->result()) > 0 ): ?>
		...
		<?php foreach($categories->result() as $category): ?>
			...
			<tr>
				...
				<td><?php echo $this->Maincategory->get_one( $category->main_cat_id )->main_cat_name; ?></td>
				...
				<?php if ( $this->ps_auth->has_access( PUBLISH )): ?>
					...
					<td>
						<?php if ( @$category->status == 1): ?>
							<button class="btn btn-sm btn-success unpublish" catid='<?php echo $category->cat_id;?>'>
							<?php echo get_msg('btn_yes'); ?></button>
						<?php else:?>
							<button class="btn btn-sm btn-danger publish" catid='<?php echo $category->cat_id;?>'>
